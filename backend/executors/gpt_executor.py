        ...
        try:
            ...
            if data.get("error"):
                return ExecutorResult(
                    error=data["error"],
                    validationSummary=ValidationSummary(passed=False, checksRun=["gpt_call"], failedChecks=["gpt_call"]),
                )
            ...
            return ExecutorResult(
                ...
                validationSummary=ValidationSummary(passed=True, checksRun=["gpt_call"]),
            )
        except Exception as exc:  # noqa: BLE001
            return ExecutorResult(
                error=f"GPT execution failed: {exc}",
                changedFiles=[],
                validationSummary=ValidationSummary(passed=False, checksRun=["gpt_call"], failedChecks=["gpt_call"]),
            )
